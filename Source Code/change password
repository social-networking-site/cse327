			...
			<div class="l_pass">
				<form action="" method="post">
					<div class="input-group">
						<span class="input-group-addo"><i class="glyphicon glyphicon-lock"></i></span>
					...
					<div class="input-group">
						<span class="input-group-addo"><i class="glyphicon glyphicon-lock"></i></span>
						<input id="password" type="password" class="form-control" name="pass1" placeholder="Re-Password" required>
					</div><br>
	...
	if (isset($_POST['change'])) 
	{
		...
		if($pass == $pass1)
		{
			if(strlen($pass) >= 6 && strlen($pass) <= 60)
            {
		        ...
		        echo"<script>alert('Your Pass Change Succes')</script>";
		...
		{
			 echo"<script>alert('Your Pass should be greather then 6 word')</script>";
		...
		{
			  echo"<script>alert('Your Pass Did not match')</script>";
